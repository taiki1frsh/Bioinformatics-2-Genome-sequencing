...
pub fn debruijn_from_kmers<'a>(set_of_kmers: &'a Vec<&str>) -> HashMap<Vec<&'a str>, Vec<Vec<&'a str>>> {
    ...
    for i in 0..set_num {
        ...
        if !pre_deb_elements.contains(&pre_kmer) { 
            pre_deb_elements.insert(count, pre_kmer.clone());
        } else {
            continue;
        }
...
fn test_debruijn_from_kmers() {
    ...
    let _deb_map = debruijn_from_kmers(&set_of_kmers);
}